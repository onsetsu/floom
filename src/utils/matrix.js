...
const WrongType1 = new TypeError('Error, wrong object! The object has to be a matrix or a number.');
const WrongType2 = new TypeError('Error, wrong object! The object has to be a matrix.');
const WrongType3 = new TypeError('Error, wrong object! The object has to be a function.');
const WrongDim1 = new RangeError('Error, wrong dimensions! Amount of columns of A have to be equal to the amount of rows of B.');
...
export default class Matrix {
  ...
  static Error_Message() {
    Matrix_Class_Error_Message = !(Matrix_Class_Error_Message);
  ...
  static fromArray(array) {
    ...
    const result = new Matrix(array.length, columns);
    result.data = result.data.map((x, i) => array[i]);
    return result;
  }
  ...
  static prod(M1, M2) {
    ...
    const helper = M2.transpose();
    result.data = result.data.map((rows, main_index) => rows.map((col, sub_index) => Matrix.array_mult(M1.data[main_index], helper.data[sub_index])));
    return result;
  }
   ...
   */
  unit() {
    this.data = this.data.map((rows, main_index) => rows.map((cols, sub_index) => ((sub_index === main_index) ? 1 : 0)));
  }
  ...
  diagonal(diagonal_num, filler) {
    ...
    this.data = this.data.map((rows, main_index) => rows.map((cols, sub_index) => ((sub_index === main_index) ? (diagonal_num || 1) : (filler || 0))));
  }
  ...
  transpose() {
    ...
    result.data = result.data.map((rows, main_index) => rows.map((cols, sub_index) => this.data[sub_index][main_index]));
    return result;
  }
  ...
  add(Obj) {
    if (Obj instanceof Matrix) {
      this.data = this.data.map((rows, main_index) => rows.map((cols, sub_index) => cols + (Obj.data[main_index][sub_index] || 0)));
    } else if (typeof Obj === 'number') { this.data = this.data.map((rows) => rows.map((cols) => cols + (Obj || 0))); } else { console.log(Matrix.wrong_type_error_message()); }
  }
  ...
  mult(Obj) {
    if (Obj instanceof Matrix) {
      this.data = this.data.map((rows, main_index) => rows.map((cols, sub_index) => cols * (Obj.data[main_index][sub_index] || 1)));
    } else if (typeof Obj === 'number') { this.data = this.data.map((rows) => rows.map((cols) => cols * (Obj || 1))); } else { console.log(Matrix.wrong_type_error_message()); }
  }
