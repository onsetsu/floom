...
export default class Renderer {
  ...
  drawSystem(system) {
    ...
    const inspectedParticle = system.particles[window.inspectedParticleIndex];
  ...
  drawGrid(grid) {
    ...
    for (let nIndex = 0; nIndex < numberOfNodes; nIndex++) {
      const n = grid.arr[nIndex];
      const x = Math.floor(nIndex / grid.gsizeY);
      const y = nIndex - (x * grid.gsizeY);
