...
class ToolEvent {
  ...
  getLastMiddlePointInWorld(viewport) {
    const positionInWorld = this.getPositionInWorld(viewport);
    const lastPositionInWorld = this.getLastPositionInWorld(viewport);
    return positionInWorld.add(lastPositionInWorld).mulFloat(0.5);
  }

  getDownMiddlePointInWorld(viewport) {
    const positionInWorld = this.getPositionInWorld(viewport);
    const downPositionInWorld = this.getDownPositionInWorld(viewport);
  ...
  getDistanceToPointInWorld(point, viewport) {
    const toolPositionInWorld = this.getPositionInWorld(viewport);
