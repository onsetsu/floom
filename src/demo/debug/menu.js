...
import DebugOption from '@/demo/debug/option';
...
export default class Menu {
  ...
  addPanel(panelDef, system) {
    ...
    const panel = new (panelDef.type)(panelDef.name, panelDef.label, system);
    if (panelDef.options) {
      for (let i = 0; i < panelDef.options.length; i += 1) {
    ...
    let inserted = false;
    for (let i = 1; i < this.panelMenu.children().length; i += 1) {
      const cn = this.panelMenu.children()[i];
  ...
  togglePanel(panel) {
    ...
    const active = !(panel.container.is(':visible'));
    panel.toggle(active);
  ...
  beforeRun() {
    const timeBeforeRun = window.performance.now();
  ...
  afterRun(renderer, fluidSystem) {
    const frameTime = window.performance.now() - this.debugRealTime;
    ...
    if (fluidSystem) {
      ...
      const numberOfParticles = fluidSystem.getNumberOfParticles();
