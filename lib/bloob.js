	...
	var BloobGame = Game.subclass({
		initialize: function() {
			...
			if(Bloob.debug && Bloob.debug.datGui) {
				var datGuiFolder = Bloob.debug.datGui.addFolder(42);
				datGuiFolder.open();
				map.initDatGui(datGuiFolder);
			}
			...
			function animate() {
				if(Bloob && Bloob.debug && Bloob.debug.beforeRun)
				...
				if(Bloob && Bloob.debug && Bloob.debug.afterRun)
