...
define(["physics/jello"], function(Jello) {
	...
	var CombinedRenderer = mini.Class.subclass({
		...
		pushLayer: function() {
			...
			// scale the current view into a 1 by 1 box
			this.context.scale(
				1 / env.viewport.extent.x,
				1 / env.viewport.extent.y
			);
			// move current world camera point to the coordinate system's origin
			this.context.translate(
				-env.viewport.point.x,
				-env.viewport.point.y
			);
			
			this.singlePixelExtent = env.viewport.screenToWorldCoordinates(new Jello.Vector2(1, 1)).sub(
				env.viewport.screenToWorldCoordinates(new Jello.Vector2(0, 0))
			);
		},
