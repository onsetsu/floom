...
define([
	...
	var CombinedRenderer = mini.Class.subclass({
		...
		pushLayer: function(layer) {
			...
			// move current world camera point to the coordinate system's origin
			this.context.translate(
				-env.camera.viewport.point.x,
				-env.camera.viewport.point.y
			);
		...
		withLayer: function(layer, func, ctx) {
		...
		updateCanvasWorldAABB: function() {
			var min = env.camera.screenToWorldCoordinates(new Jello.Vector2(0, env.canvas.height));
			var max = env.camera.screenToWorldCoordinates(new Jello.Vector2(env.canvas.width, 0));
